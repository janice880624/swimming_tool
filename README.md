...
- 疲勞監測（M6）：RPE 主觀疲勞量表、睡眠品質與恢復感受自評、依 RPE×訓練時長自動計算 ACWR 訓練負荷比值（急性 7 日 / 慢性 28 日），並在畫面上標示風險等級
- 教練訓練處方（M8）：教練可手動輸入文字處方；系統會依 ACWR 與近期恢復狀況自動產生提醒（例如負荷過高、恢復感受偏低）
- 基準測驗（M2 Baseline Assessment）：檢測紀錄可標記某一筆為「基準」，分析頁會自動顯示最新一筆與基準的差異百分比
